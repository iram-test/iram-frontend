...
import { TestRunDTO, CreateTestRunDTO, UpdateTestRunDTO } from '../models/test-run-dto';
...
export class TestRunService extends BaseService {
  ...
  createTestRun(testRun: CreateTestRunDTO, projectId: string): Observable<TestRunDTO> {
    return this.post<TestRunDTO>(`projects/${projectId}/test-runs`, testRun);
  }
